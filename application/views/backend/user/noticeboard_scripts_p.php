<script type="text/javascript">
	...
	function load_notic_list(){
		if(!$('#notice_list').html()){
	    	...
	    	$.ajax({
		        url: '<?php echo site_url('addons/noticeboard/load_notice_list_p/'.$course_details['id']) ?>',
	...
	function load_notic_form_p(){
    	$.ajax({
	        url: '<?php echo site_url('addons/noticeboard/load_noticeboard_form_p') ?>',
	        success: function(response){
	        	$('#noticeboard_form_p').html(response);
	        }
	    });
	}
	...
	function add_new_notice(btn){
		...
		if(notice_title != "" && notice_description != "") {
		    $ajax({
		        url: '<?php echo site_url('addons/noticeboard/add_notice_programs/'.$course_details['id']) ?>',
		        type: 'post',
	...
	function update_notice(btn, notice_id){
		...
		if(notice_title != "" && notice_description != "") {
		    $.ajax({
		        url: '<?php echo site_url('addons/noticeboard/edit_notice_p/') ?>' + notice_id + '/update',
		        type: 'post',
	...
	function show_added_notice(notice_id){
		$.ajax({
	        url: '<?php echo site_url('addons/noticeboard/load_single_notice_p/'); ?>' + notice_id,
	        success: function(response){
