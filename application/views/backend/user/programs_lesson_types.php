        ...
        <div class="col-md-12">
            <div class="form-group">
                <select class="select2" id="course_id_for_lesson" name="course_id_for_lesson">
                    <option value=""><?= get_phrase('select_a_programs'); ?></option>
                    <?php foreach($this->crud_model->get_course_by_course_type('general')->result_array() as $programs): ?>
...
<?php else: ?>
    <div class="alert alert-info" role="alert">
        Course: <strong><?= $this->crud_model->get_programs_by_id($param2)->row('title'); ?></strong>
    </div>
...
<script type="text/javascript">
    function showLessonAddModal() {
        ...
        if(course_id > 0){
            var url = "<?php echo site_url('modal/popup/programs_lesson_add/'); ?>/"+course_id+'/'+$("input[name=lesson_type]:checked").val();
            showAjaxModal(url, '<?php echo get_phrase('add_new_lesson'); ?>');
        }else{
            error_notify('<?php echo get_phrase('please_select_a_programs'); ?>');
        }
