<div class="row ">
    <div class="col-xl-12">
        <div class="card">
            <div class="card-body">
                <h4 class="page-title"> <i class="mdi mdi-apple-keyboard-command title_icon"></i> <?php echo get_phrase('organization_payouts'); ?></h4>
            </div> <!-- end card body-->
        </div> <!-- end card -->
    </div><!-- end col-->
</div>
                ...
                <div class="tab-content">
                    <div class="tab-pane show active" id="completed-b1">
                        <div class="row justify-content-md-center">
                            <div class="col-xl-6">
                                <form class="form-inline" action="<?php echo site_url('admin/organization_payout/filter_by_date_range') ?>" method="get">
                                    <div class="col-xl-10">
                                            ...
                                            <td style="text-align: center;">
                                              <?php if ($pending_payout['status'] == 0): ?>
                                                <?php if ($paypal_keys[0]['production_client_id'] != ""): ?>
                                                  <form action="<?php echo site_url('admin/paypal_checkout_for_organization_revenue'); ?>" method="post">
                                                    ...
                                                    <input type="hidden" name="organization_name"      value="<?php echo $pending_payout_user_data['first_name'].' '.$pending_payout_user_data['last_name']; ?>">
                                                ...
                                                <?php else: ?>
                                                  <button type="button" class = "btn btn-outline-danger btn-sm btn-rounded" name="button" onclick="alert('<?php echo get_phrase('this_organization_has_not_provided_valid_paypal_client_id'); ?>')"><?php echo get_phrase('pay_with_paypal'); ?></button>
                                                <?php endif; ?>
                                                <?php if ($stripe_keys[0]['public_live_key'] != "" && $stripe_keys[0]['secret_live_key']): ?>
                                                  <?php include 'stripe_payment_gateway_form.php'; ?>
                                                <?php else: ?>
                                                  <button type="button" class = "btn btn-outline-danger btn-sm btn-rounded" name="button" onclick="alert('<?php echo get_phrase('this_organization_has_not_provided_valid_public_key_or_secret_key'); ?>')"><?php echo get_phrase('pay_with_stripe'); ?></button>
                                                <?php endif; ?>
                                              <?php else: ?>
