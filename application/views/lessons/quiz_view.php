...
<div id="quiz-body">
    <div class="" id="quiz-header">
        ...
        <?php if (count($quiz_questions->result_array()) > 0): ?>
            <button type="button" name="button" class="btn btn-get-started mt-2" style="color: #fff;" onclick="getStarted(1)"><?php echo get_phrase("get_started"); ?></button>
        <?php endif; ?>
    </div>
        ...
        <?php if (count($quiz_questions->result_array()) > 0): ?>
            ...
            ?>
                ...
                <div class="hidden" id = "question-number-<?php echo $key+1; ?>">
                    ...
                    <button type="button" name="button" class="btn btn-sign-up btn-get-started mt-2 mb-2" id = "next-btn-<?php echo $quiz_question['id'];?>" style="color: #fff;" <?php if(count($quiz_questions->result_array()) == $key+1):?>onclick="submitQuiz()"<?php else: ?>onclick="showNextQuestion('<?php echo $key+2; ?>')"<?php endif; ?> disabled><?php echo count($quiz_questions->result_array()) == $key+1 ? get_phrase("check_result") : get_phrase("submit_and_next"); ?></button>
                </div>
            <?php endforeach; ?>
        <?php endif; ?>
    </form>
