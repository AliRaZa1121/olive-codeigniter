...
class Crud_model extends CI_Model
{
    ...
    public function all_categories($param1 = "")
    {
        if ($param1 != "") {
            $this->db->where('id', $param1);
        }
    ...
    public function enrol_history_p($course_id = "")
    {
        if ($course_id > 0) {
            return $this->db->get_where('enrol', array('course_id' => $course_id));
        } else {
    ...
    public function update_organization_settings()
    {
        if (isset($_POST['allow_organization'])) {
            $data['value'] = html_escape($this->input->post('allow_organization'));
            $this->db->where('key', 'allow_organization');
            $this->db->update('settings', $data);
        }

        if (isset($_POST['organization_revenue'])) {
            $data['value'] = html_escape($this->input->post('organization_revenue'));
            $this->db->where('key', 'organization_revenue');
            $this->db->update('settings', $data);
        }

        if (isset($_POST['acinstructor_application_note'])) {
            $data['value'] = html_escape($this->input->post('acinstructor_application_note'));
            $this->db->where('key', 'acinstructor_application_note');
            $this->db->update('settings', $data);
        }
    }

    public function get_lessons_p($type = "", $id = "")
    {
    ...
    public function add_programs($param1 = "")
    {
        $outcomes = $this->trim_and_return_json($this->input->post('outcomes'));
        $requirements = $this->trim_and_return_json($this->input->post('requirements'));
        ...
        if ($data['status'] == 'approved') {
            $this->session->set_flashdata('flash_message', get_phrase('consultancy_programs_added_successfully'));
        } elseif ($data['status'] == 'pending') {
            $this->session->set_flashdata('flash_message', get_phrase('consultancy_programs_added_successfully') . '. ' . get_phrase('please_wait_untill_Admin_approves_it'));
        } elseif ($data['status'] == 'draft') {
            $this->session->set_flashdata('flash_message', get_phrase('your_consultancy_programs_has_been_added_to_draft'));
        }

        $this->session->set_flashdata('flash_message', get_phrase('consultancy_programs_has_been_added_successfully'));
        return $course_id;
    }
    ...
    public function add_shortcut_programs($param1 = "")
    {
        $data['course_type'] = html_escape($this->input->post('course_type'));
        $data['title'] = html_escape($this->input->post('title'));
        $data['outcomes'] = '[]';
    ...
    public function update_programs($course_id, $type = "")
    {
        $course_details = $this->get_course_by_id($course_id)->row_array();
    ...
    public function change_programs_status($status = "", $programs_id = "")
    {
        if ($status == 'active') {
            if ($this->session->userdata('admin_login') != true) {
                redirect(site_url('login'), 'refresh');
    ...
    public function get_programs_thumbnail_url($course_id, $type = 'course_thumbnail')
    {
        // Course media placeholder is coming from the theme config file. Which has all the placehoder for different images. Choose like course type.
        $course_media_placeholders = themeConfiguration(get_frontend_settings('theme'), 'course_media_placeholders');
        // if (file_exists('uploads/thumbnails/course_thumbnails/'.$type.'_'.get_frontend_settings('theme').'_'.$course_id.'.jpg')){
    ...
    public function delete_programs($programs_id = "")
    {
        $programs_type = $this->get_course_by_id($programs_id)->row('course_type');

        $this->db->where('id', $programs_id);
        $this->db->delete('programs');

        if ($programs_type == 'general') {
            // DELETE ALL THE LESSONS OF THIS COURSE FROM LESSON TABLE
            $lesson_checker = array('programs_id' => $programse_id);
            $this->db->delete('lesson', $lesson_checker);

            // DELETE ALL THE section OF THIS COURSE FROM section TABLE
            $this->db->where('programs_id', $programs_id);
            $this->db->delete('section');
        } elseif ($programs_type == 'scorm') {
            $this->load->model('addons/scorm_model');
            $scorm_query = $this->scorm_model->get_scorm_curriculum_by_course_id($programs_id);
            ...
            if ($scorm_query->num_rows() > 0) {
                //deleted previews course directory
                $this->scorm_model->deleteDir('uploads/scorm/programs/' . $scorm_query->row('identifier'));
            }
        }
    }
    ...
    public function get_status_wise_programs_for_instructor($status = "")
    {
        $user_id = 0;
        $multi_instructor_programs_ids = [];
        if ($this->session->userdata('user_login') == true) {
            $user_id = $this->session->userdata('user_id');
            $multi_instructor_programs_ids = $this->multi_instructor_programs_ids_for_an_instructor($user_id);
        }
        if ($status != "") {
            if ($user_id != 0) {
                $this->db->where('user_id', $user_id);
            ...
            if ($multi_instructor_programs_ids && count($multi_instructor_programs_ids)) {
                $this->db->or_where_in('id', $multi_instructor_programs_ids);
            }
            $courses = $this->db->get('course');
        } else {
            $this->db->where('type', 2);
            ...
            if ($multi_instructor_programs - ids && count($multi_instructor_programs_ids)) {
                $this->db->or_where_in('id', $multi_instructor_programs_ids);
            }
            $courses['pending'] = $this->db->get('course');
            $this->db->where('type', 2);
    ...
    public function get_instructor_wise_programs($instructor_id = "", $return_as = "")
    {
        // GET COURSE IDS FOR MULTI INSTRUCTOR
        $multi_instructor_course_ids = $this->multi_instructor_programs_ids_for_an_instructor($instructor_id);
    ...
    public function add_programs_section($course_id)
    {
        $data['title'] = html_escape($this->input->post('title'));
        $data['programs_id'] = $course_id;
        $this->db->insert('section', $data);
        $section_id = $this->db->insert_id();
    ...
    public function edit_programs_section($section_id)
    {
        $data['title'] = $this->input->post('title');
        $this->db->where('id', $section_id);
        $this->db->update('section', $data);
    ...
    public function delete_programs_section($course_id, $section_id)
    {
        $this->db->where('id', $section_id);
        $this->db->delete('section');
        ...
        if (sizeof($previous_sections) > 0) {
            ...
            $updater['section'] = json_encode($new_section);
            $this->db->where('id', $course_id);
            $this->db->update('programs', $updater);
        }
    }
    ...
    public function get_section_p($type_by, $id)
    {
    ...
    public function add_lesson_programs()
    {

        $data['programs_id'] = html_escape($this->input->post('programs_id'));
        $data['title'] = html_escape($this->input->post('title'));
        $data['section_id'] = html_escape($this->input->post('section_id'));
        ...
        if ($lesson_type == 'video') {
            ...
            if ($lesson_provider == 'youtube' || $lesson_provider == 'vimeo') {
                if ($this->input->post('video_url') == "" || $this->input->post('duration') == "") {
                    $this->session->set_flashdata('error_message', get_phrase('invalid_list_url_and_duration'));
                    redirect(site_url(strtolower($this->session->userdata('role')) . '/programs_form/edit_programs/' . $data['programs_id']), 'refresh');
                }
                $data['video_url'] = html_escape($this->input->post('video_url'));
                ...
                $this->session->set_flashdata('error_message', get_phrase('invalid_list_provider'));
                redirect(site_url(strtolower($this->session->userdata('role')) . '/programs_form/edit_programs/' . $data['programs_id']), 'refresh');
            }
            ...
            if ($this->input->post('amazon_s3_duration') == "") {
                $this->session->set_flashdata('error_message', get_phrase('invalid_list_duration'));
                redirect(site_url(strtolower($this->session->userdata('role')) . '/programs_form/edit_programs/' . $data['programs_id']), 'refresh');
            }
    ...
    public function edit_lesson_programs($lesson_id)
    {
    ...
    public function delete_lesson_programs($lesson_id)
    {
        $this->db->where('id', $lesson_id);
        $this->db->delete('lesson');
    }
    ...
    public function update_frontend_settings()
    {
        ...
        $data['value'] = html_escape($this->input->post('mid_title'));
        $this->db->where('key', 'mid_title');
        $this->db->update('frontend_settings', $data);

        $data['value'] = html_escape($this->input->post('mid_sub_title'));
        $this->db->where('key', 'mid_sub_title');
        $this->db->update('frontend_settings', $data);
    ...
    public function handleWishList_p($programs_id)
    {
        $wishlists = array();
        $user_details = $this->user_model->get_user($this->session->userdata('user_id'))->row_array();
        if ($user_details['wishlist'] == "") {
            array_push($wishlists, $programs_id);
        } else {
            $wishlists = json_decode($user_details['wishlist']);
            if (in_array($programs_id, $wishlists)) {
                $container = array();
                foreach ($wishlists as $key) {
                    if ($key != $programs_id) {
                        array_push($container, $key);
                ...
                $wishlists = $container;
                // $key = array_search($programs_id, $wishlists);
                // unset($wishlists[$key]);
            } else {
                array_push($wishlists, $programs_id);
            }
    ...
    public function is_added_to_wishlist_programs($programs_id = "")
    {
        if ($this->session->userdata('user_login') == 1) {
            $wishlists = array();
            $user_details = $this->user_model->get_user($this->session->userdata('user_id'))->row_array();
    ...
    public function getWishLists_p($user_id = "")
    {
        if ($user_id == "") {
            $user_id = $this->session->userdata('user_id');
        }
    ...
    public function get_latest_10_course($id = "")
    {
        if (addon_status('scorm_course')) {
    ...
    public function category_hierarchy($id = "", $list = "")
    {
        $category = $this->get_category_details_by_id($id)->row_array();

        if ($category) {

            if ($category['parent'] > 0) {
                $list .= '<li ><a href="' . site_url('home/courses?category=' . $category['slug']) . '">  ' . $category['name'] . '</a> <i class="fas fa-chevron-right"></i> </li>';

                return $this->category_hierarchy($category['parent'], $list);
                // return $list;
            } else {
                $list .= '<li ><a href="' . site_url('home/courses?category=' . $category['slug']) . '">  ' . $category['name'] . '</a>  </li>';
    ...
    public function category_hierarchy_p($id = "", $list = "")
    {
        $category = $this->get_category_details_by_id($id)->row_array();
        ...
        if ($category) {
            ...
            if ($category['parent'] > 0) {
                $list .= '<li ><a href="' . site_url('home/programs_page?category=' . $category['slug']) . '">  ' . $category['name'] . '</a> <i class="fas fa-chevron-right"></i> </li>';

                return $this->category_hierarchy_p($category['parent'], $list);
                // return $list;
            } else {
                $list .= '<li ><a href="' . site_url('home/programs_page?category=' . $category['slug']) . '">  ' . $category['name'] . '</a>  </li>';
    ...
    public function enrol_to_free_programs($course_id = "", $user_id = "")
    {
        $course_details = $this->get_course_by_id($course_id)->row_array();
        if ($course_details['is_free_course'] == 1) {
            $data['course_id'] = $course_id;
            $data['user_id'] = $user_id;
            if ($this->db->get_where('enrol', $data)->num_rows() > 0) {
                $this->session->set_flashdata('error_message', get_phrase('student_has_already_been_enrolled_to_this_programs'));
            } else {
                $data['date_added'] = strtotime(date('D, d-M-Y'));
                $this->db->insert('enrol', $data);
                $this->session->set_flashdata('flash_message', get_phrase('successfully_enrolled'));
            ...
            $this->session->set_flashdata('error_message', get_phrase('this_programs_is_not_free_at_all'));
            redirect(site_url('home/programs_page/' . slugify($course_details['title']) . '/' . $course_id), 'refresh');
        }
    }
    ...
    public function get_programs_by_wishlists()
    {
        $wishlists = $this->getWishLists();
        if (sizeof($wishlists) > 0) {
            $this->db->where_in('id', $wishlists);
    ...
    public function get_total_duration_of_lesson_by_programs_id($course_id)
    {
        $total_duration = 0;
        $lessons = $this->crud_model->get_lessons_p('course', $course_id)->result_array();
        foreach ($lessons as $lesson) {
            if ($lesson['lesson_type'] != "other" && $lesson['lesson_type'] != "text") {
                $time_array = explode(':', $lesson['duration']);
                $hour_to_seconds = $time_array[0] * 60 * 60;
    ...
    {
        $course_ids = $this->get_instructor_wise_programs($instructor_id, 'simple_array');
    ...
    // version 1.4
    public function filter_course($selected_type = "", $selected_category_id = "", $selected_price = "", $selected_level = "", $selected_language = "", $selected_rating = "")
    {
        //echo $selected_category_id.' '.$selected_price.' '.$selected_level.' '.$selected_language.' '.$selected_rating;

        $course_ids = array();
        //types of programs and course
        if ($selected_type != "all") {
            if ($selected_type == "course") {
                $this->db->where('type', 1);
            } elseif ($selected_type == "consultancy_programs") {
                $this->db->where('type', 2);
            }
        }
        ...
        //language of programs and course
    ...
    // version 1.4
    public function filter_programs($selected_category_id = "", $selected_price = "", $selected_level = "", $selected_language = "", $selected_rating = "")
    {
        //echo $selected_category_id.' '.$selected_price.' '.$selected_level.' '.$selected_language.' '.$selected_rating;
        ...
        $this->db->where('status', 'active');
        $courses = $this->db->get('programs')->result_array();

        foreach ($courses as $course) {
            if ($selected_rating != "all") {
                $total_rating = $this->get_ratings('programs', $course['id'], true)->row()->rating;
                $number_of_ratings = $this->get_ratings('programs', $course['id'])->num_rows();
                if ($number_of_ratings > 0) {
                    $average_ceil_rating = ceil($total_rating / $number_of_ratings);
                    if ($average_ceil_rating == $selected_rating) {
                        array_push($course_ids, $course['id']);
    ...
    public function get_coursesV2($category_id = "", $sub_category_id = "", $instructor_id = 0)
    {
        if ($category_id > 0 && $sub_category_id > 0 && $instructor_id > 0) {
    ...
    public function get_programs($category_id = "", $sub_category_id = "", $instructor_id = 0)
    {
        if ($category_id > 0 && $sub_category_id > 0 && $instructor_id > 0) {
            ...
            return $this->db->get_where('programs', array('category_id' => $category_id, 'sub_category_id' => $sub_category_id));
        } else {
            return $this->db->get('programs');
        }
    }
    ...
    public function filter_students_for_quizes($student_id)
    {
        $multi_student_course_ids = array();
        if ($student_id != "all") {
            $multi_student_course_ids = $this->multi_instructor_quizes_ids_for_an_student($student_id);
        }

        if ($student_id != "all") {
            $this->db->where('user_id', $student_id);
            if ($multi_student_course_ids && count($multi_student_course_ids)) {
                $this->db->or_where_in('id', $multi_student_course_ids);
            }
        }
    }

    public function filter_quizes_for_backend($instructor_id)
    {
        // MULTI INSTRUCTOR COURSE IDS
        $multi_instructor_quizes_ids = array();
        if ($instructor_id != "all") {
            $multi_instructor_course_ids = $this->multi_instructor_quizes_ids_for_an_student($instructor_id);
        }

        if ($instructor_id != "all") {
            $this->db->where('user_id', $instructor_id);
            if ($multi_instructor_quizes_ids && count($multi_instructor_quizes_ids)) {
                $this->db->or_where_in('id', $multi_instructor_quizes_ids);
            }
        }

        if ($status != "all") {
            $this->db->get('quiz_answers')->result_array();
        }
        return $this->db->get('quiz_answers')->result_array();
    }

    public function filter_programs_for_backend($category_id, $instructor_id, $price, $status)
    {
        // MULTI INSTRUCTOR COURSE IDS
        $multi_instructor_programs_ids = array();
        if ($instructor_id != "all") {
            $multi_instructor_programs_ids = $this->multi_instructor_programs_ids_for_an_instructor($instructor_id);
        }
        if ($category_id != "all") {
            $this->db->where('sub_category_id', $category_id);
        }
    ...
    public function get_free_and_paid_programs($price_status = "", $instructor_id = 0)
    {
        $instructor_id = 0;
        if ($this->session->userdata('user_login') == true) {
    ...
    public function get_quiz_answer_student($quiz_id)
    {
    ...
    public function get_programs_by_course_type($type = "")
    {
        if ($type != "") {
            $this->db->where('course_type', $type);
        }
    ...
    public function multi_instructor_quizes_ids_for_an_student($instructor_id)
    {
        $quizes_id = array();
        ...
        foreach ($multi_instructor_courses as $key => $multi_instructor_course) {
            $exploded_user_ids = explode(',', $multi_instructor_course['user_id']);
            if (in_array($instructor_id, $exploded_user_ids)) {
                array_push($quizes_id, $multi_instructor_course['id']);
            }
        }
        return $quizes_id;
    }

    public function multi_instructor_programs_ids_for_an_instructor($instructor_id)
    {
        $programs_ids = array();
        $multi_instructor_programs = $this->db->get_where('course', array('multi_instructor' => 1))->result_array();
        foreach ($multi_instructor_programs as $key => $multi_instructor_programs) {
            $exploded_user_ids = explode(',', $multi_instructor_programs['user_id']);
            if (in_array($instructor_id, $exploded_user_ids)) {
                array_push($programs_ids, $multi_instructor_programs['id']);
            }
        }
        return $programs_ids;
    }
    ...
    public function add_sample_video()
    {

        if (!file_exists('uploads/thumbnails/sample_videos')) {
            mkdir('uploads/thumbnails/sample_videos', 0777, true);
        }
        $url = "";
        if ($_FILES['image']['name'] != "") {
            $url = 'uploads/thumbnails/sample_videos/' . strtotime('now') . $_FILES['image']['name'];
            move_uploaded_file($_FILES['image']['tmp_name'], $url);
        }

        if (!file_exists('uploads/videos/sample_videos')) {
            mkdir('uploads/videos/sample_videos', 0777, true);
        }
        $video_url = "";
        if ($_FILES['video']['name'] != "") {
            $video_url = 'uploads/videos/sample_videos/' . strtotime('now') . $_FILES['video']['name'];
        ...
        $this->db->insert('sample_videos', $data);
        return $this->db->insert_id();
    }

    public function get_sample_video_by_id($id)
    {
        return $this->db->get_where('sample_videos', array('id' => $id));
    }

    public function update_sample_video($id)
    {
        $sample_video = $this->get_sample_video_by_id($id)->row_array();
        if (!file_exists('uploads/thumbnails/sample_videos')) {
            mkdir('uploads/thumbnails/sample_videos', 0777, true);
        }
        $url = "";
        ...
        $this->db->where('id', $sample_video['id']);
        $this->db->update('sample_videos', $data);
    }

    public function get_sample_videos()
    {
        return $this->db->get('sample_videos')->result_array();
    }

    public function delete_sample_video($id)
    {
        $this->db->where('id', $id);
        $this->db->delete('sample_videos');
    }

    public function change_status_sample_video($status, $id)
    {
        $data['status'] = $status;
        $this->db->where('id', $id);
        $this->db->update('sample_videos', $data);
    ...
    public function get_sample_videos_by_course_id($id)
    {
        $this->db->where('course_id', $id);
        return $this->db->get('sample_videos');
    }

    public function get_sample_videos_by_programs_id($id)
    {
    ...
    // public function get_all_report_count($id){
    ...
    public function get_reports()
    {
        ...
        // $query = $this->db
        // ->select("reports.id as id, reports.reason as reason, users.first_name as first_name,users.last_name as last_name, users.is_instructor as is_instructor ,course.title as course_name")
        //     ->from ("reports")
        //     ->join('course', 'course.id = reports.course_id')
        //     ->join('users', 'users.id = reports.user_id')
        //     ->get();
        $this->db->where('terminate', 0);
        $reports = $this->db->get('reports')->result_array();
        foreach ($reports as $key => $report) {
            $user = $this->user_model->get_user($report['user_id'])->row_array();
            $reports[$key]['first_name'] = $user['first_name'];
            $reports[$key]['last_name'] = $user['last_name'];
            $reports[$key]['is_instructor'] = $user['is_instructor'];

            $course = $this->get_course_by_id($report['object_id'])->row_array();
            // $reports[$key]['course'] = $course;
            $reports[$key]['course_name'] = $course['title'];
        }
    ...
    public function add_warning()
    {
        ...
        $data['user_id'] = $_POST['user_id'];
        $data['warning'] = $_POST['warning'];
        $this->db->insert('warnings', $data);
        $this->session->set_flashdata('flash_message', 'Warning add successfully against this user');
    }
    ...
    public function get_warning_count()
    {
        $user_id = $this->session->userdata('user_id');
        $this->db->where('user_id', $user_id);
        $warning = $this->db->get('warnings')->result_array();
    ...
    public function warning_reply()
    {
        $data['reply'] = $_POST['reply'];
        $data['resolved'] = 1;
        $this->db->where('id', $_POST['id']);
        $this->db->update('warnings', $data);
        $this->session->set_flashdata('flash_message', 'Replied to report successfully');
    }

    public function warning_status($id)
    {
        $data['read_status'] = 1;
        $this->db->where('id', $id);
        $this->db->update('warnings', $data);
    ...
    {
        $this->db->where('read_status', null);
        $report = $this->db->get('reports')->result_array();
    ...
    public function terminate_report($id)
    {
        $data['terminate'] = 1;
        $this->db->where('id', $id);
        $this->db->update('reports', $data);
        $this->session->set_flashdata('flash_message', 'Report terminated successfully');
    }

    public function submit_answer_api($question_id = 0, $answer = "", $lesson_id = 0, $user_id = 0)
    {
        $data['user_id'] = $user_id;
        $data['question_id'] = $question_id['id'];
        $data['answer'] = json_encode($answer['answer']);
    ...
    public function get_quiz_answers_filter($question_id)
    {

        $e = $_GET['student_id'];
        $this->db->where('question_id', $question_id);
        $this->db->where('user_id', $e);
        return $this->db->get('quiz_answers');
    }

    public function add_marks($answer_id)
    {
    ...
    public function get_contents_details_by_id($id)
    {
        return $this->db->get_where('contents', array('id' => $id))->row_array();
    }

    public function get_contents_id($slug = "")
    {
        $contents_details = $this->db->get_where('contents', array('slug' => $slug))->row_array();
        return $contents_details['id'];
    }

    public function add_content($type)
    {
        $data['name'] = html_escape($this->input->post('name'));
        $data['description'] = html_escape($this->input->post('description'));
        $data['slug'] = slugify(html_escape($this->input->post('name')));
        ...
        $previous_data = $this->db->get('contents')->num_rows();
        ...
        if ($previous_data == 0) {
            ...
            if ($_FILES['thumbnail']['name'] == "") {
                ...
                $data['thumbnail'] = md5(rand(10000000, 20000000)) . '.jpg';
                move_uploaded_file($_FILES['thumbnail']['tmp_name'], 'uploads/thumbnails/category_thumbnails/' . $data['thumbnail']);
            }
            $data['date_added'] = strtotime(date('D, d-M-Y'));
            $data['last_modified'] = strtotime(date('D, d-M-Y'));
    ...
    public function edit_content($type, $id)
    {
        $data['name'] = html_escape($this->input->post('name'));
        $data['description'] = html_escape($this->input->post('description'));
        $data['slug'] = slugify(html_escape($this->input->post('name')));
        ...
        $this->db->where('name', $data['name']);
        $this->db->or_where('slug', $data['slug']);
        $previous_data = $this->db->get('contents')->result_array();
    ...
    public function get_organizations_front($limit = null)
    {
    ...
    public function get_books_front($limit = null)
    {
    ...
    ///  organizations
    public function get_organizations($id = "")
    {
        if ($id != "") {
            $this->db->where('id', $id);
        }
    ...
    public function add_organzation()
    {
        ...
        $previous_data = $this->db->get('organizations')->num_rows();
        ...
        if ($previous_data == 0) {
            ...
            if ($_FILES['banner']['name'] == "") {
                $data['banner'] = 'category-thumbnail.png';
            } else {
                $data['banner'] = md5(rand(10000000, 20000000)) . '.jpg';
                move_uploaded_file($_FILES['banner']['tmp_name'], 'uploads/thumbnails/category_thumbnails/' . $data['banner']);
            }
            ...
            if ($_FILES['logo']['name'] == "") {
                $data['logo'] = 'category-thumbnail.png';
            } else {
                $data['logo'] = md5(rand(10000000, 20000000)) . '.jpg';
                move_uploaded_file($_FILES['logo']['tmp_name'], 'uploads/thumbnails/category_thumbnails/' . $data['logo']);
            }
    ...
    public function edit_organzation($id)
    {
        $data['name'] = html_escape($this->input->post('name'));
        $data['description'] = html_escape($this->input->post('description'));
        $data['slug'] = slugify(html_escape($this->input->post('name')));
        ...
        $this->db->where('name', $data['name']);
        $this->db->or_where('slug', $data['slug']);
        $previous_data = $this->db->get('organizations')->result_array();
    ...
    public function get_organization_details_by_id($id)
    {
        return $this->db->get_where('organizations', array('id' => $id))->row_array();
    }

    public function delete_organzation($id)
    {
        $this->db->where('id', $id);
    ...
    public function add_book()
    {
        ...
        $previous_data = $this->db->get('books')->num_rows();
        ...
        if ($previous_data == 0) {
            ...
            if ($_FILES['file']['name'] == "") {
                ...
                $data['file'] = md5(rand(10000000, 20000000)) . '.pdf';
                move_uploaded_file($_FILES['file']['tmp_name'], 'uploads/thumbnails/category_thumbnails/' . $data['file']);
            }
    ...
    public function edit_book($id)
    {
        ...
        $this->db->where('name', $data['name']);
        $this->db->or_where('slug', $data['slug']);
        $previous_data = $this->db->get('books')->result_array();
    ...
    {
        $book = $this->db->get_where('books', array('id' => $id))->row_array();
    ...
    public function get_all_pages_list($page = null)
    {
        if ($page == null) {
            $this->db->select('content_settings.page_name');
            $this->db->group_by('page_name');
            return $this->db->get('content_settings');
        } else {
    ...
    public function get_content_settings($page_name, $section)
    {

        $this->db->where('page_name', $page_name);
        $this->db->where('section', $section);
        return $this->db->get('content_settings')->row_array();
    }

    public function get_sub_content_settings($sub_section)
    {
        $this->db->where('sub_section', $sub_section);
        return $this->db->get('content_settings_sub')->row_array();
    }

    public function get_sub_content_settings_array()
    {
        return $this->db->get('content_settings_sub')->result_array();
    }


    public function content_settings_update($page_name, $section)
    {
        /// sub section 
        $sub_section = html_escape($this->input->post('sub_section'));
        if ($sub_section != null) {
            //checking parent
            $this->db->where('page_name', $page_name);
            $this->db->where('section', $section);
            $content = $this->db->get('content_settings')->num_rows();
            if ($content == 0) {
            ...
            $data['content_setting_id'] = $content['id'];
            // image
            if (isset($_FILES['sub_section_image']) && $_FILES['sub_section_image']['name'] != "") {
                $data['sub_section_image'] = md5(rand(1000, 100000)) . '.jpg';
                move_uploaded_file($_FILES['sub_section_image']['tmp_name'], 'uploads/system/' . $data['sub_section_image']);
            }

            $this->db->where('content_setting_id', $content['id']);
            $this->db->where('sub_section', $sub_section);
            $content_settings_sub = $this->db->get('content_settings_sub')->num_rows();


            if ($content_settings_sub == 0) {
                $data['date_added'] = strtotime(date('D, d-M-Y'));
                $data['last_modified'] = strtotime(date('D, d-M-Y'));
                $this->db->insert('content_settings_sub', $data);
            } else {
                $data['last_modified'] = strtotime(date('D, d-M-Y'));
                $this->db->where('content_setting_id', $content['id']);
                $this->db->where('sub_section', $sub_section);
                $this->db->update('content_settings_sub', $data);
            }
        } else {
            ////main section
            $data['title'] = html_escape($this->input->post('title'));
            $data['sub_title'] = html_escape($this->input->post('sub_title'));
            $data['button_text'] = html_escape($this->input->post('button_text'));
            $data['button_link'] = html_escape($this->input->post('button_link'));
            $data['description_1'] = html_escape($this->input->post('description_1'));
            $data['description_2'] = html_escape($this->input->post('description_2'));
            // image
            if (isset($_FILES['image']) && $_FILES['image']['name'] != "") {
                $data['image'] = md5(rand(1000, 100000)) . '.jpg';
                move_uploaded_file($_FILES['image']['tmp_name'], 'uploads/system/' . $data['image']);
            }
            ...
            if ($content == 0) {
                ...
                $this->db->where('page_name', $page_name);
                $this->db->where('section', $section);
                $this->db->update('content_settings', $data);
            }
        }
    }
