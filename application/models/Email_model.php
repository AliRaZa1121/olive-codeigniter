...
class Email_model extends CI_Model
{
    ...
    public function password_reset_email($new_password = '', $email = '')
    {
    ...
    public function bundle_purchase_notification_admin($bundle_details = "", $admin_details = "", $bundle_creator_details = "", $student_details = "")
    {
    ...
    public function bundle_purchase_notification_bundle_creator($bundle_details = "", $admin_details = "", $bundle_creator_details = "", $student_details = "")
    {
        $email_msg = "<h2>" . $bundle_details['title'] . "</h2>";
    ...
    public function bundle_purchase_notification_student($bundle_details = "", $admin_details = "", $bundle_creator_details = "", $student_details = "")
    {
        $email_msg = "<h2>" . $bundle_details['title'] . "</h2>";
    ...
    public function send_mail_on_suspension($id)
    {
        $user = $this->user_model->get_user($id)->row_array();

        $email_msg = "<h2> Dear " . $user['username'] . "</h2>";
        $email_msg .= "<p> You're Suspened. Please contact admin to activate your account </p>";

        $email_data['subject'] = 'User Suspension Notification';
        $email_data['from'] = get_settings('system_email');
        $email_data['to'] = $user['email'];
        $email_data['to_name'] = $user['first_name'] . ' ' . $user['last_name'];
        $email_data['message'] = $email_msg;
        $email_template = $this->load->view('email/common_template', $email_data, true);
        $this->send_smtp_mail($email_template, $email_data['subject'], $email_data['to'], $email_data['from']);
    }

    public function send_mail_on_account_verify($id)
    {
        $user = $this->user_model->get_user($id)->row_array();

        $email_msg = "<h2> Dear " . $user['username'] . "</h2>";
        $email_msg .= "<p>Your account has been verified, you can now purchase your desired programs</p>";
        $email_data['subject'] = 'Account Activation Notification';
        $email_data['from'] = get_settings('system_email');
        $email_data['to'] = $user['email'];
        $email_data['to_name'] = $user['first_name'] . ' ' . $user['last_name'];
        $email_data['message'] = $email_msg;
    ...
    public function send_mail_on_banned($id)
    {
        $user = $this->user_model->get_user($id)->row_array();

        $email_msg = "<h2> Dear " . $user['username'] . "</h2>";
        $email_msg .= "<p> You're banned. Please contact admin to activate your account </p>";

        $email_data['subject'] = 'User Banned Notification';
        $email_data['from'] = get_settings('system_email');
        $email_data['to'] = $user['email'];
        $email_data['to_name'] = $user['first_name'] . ' ' . $user['last_name'];
        $email_data['message'] = $email_msg;
    ...
    public function send_notice($notice_id = "", $course_id = "")
    {
        $course_details = $this->crud_model->get_course_by_id($course_id)->row_array();
    ...
    public function send_notice_p($notice_id = "", $course_id = "")
    {
    ...
    public function live_class_invitation_mail($to = "")
    {
