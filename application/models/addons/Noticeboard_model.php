...
class Noticeboard_model extends CI_Model
{
    ...
    public function add_notice_programs($course_id = ""){
        if($this->is_valid_user($course_id)->num_rows() > 0){
            $data['title'] = htmlspecialchars($this->input->post('notice_title'));
	        $data['description'] = htmlspecialchars($this->input->post('notice_description'));
	        $data['status'] = 1;
	        ...
	        if($this->input->post('mail_to_students') > 0){
	        	$send_mail = $this->email_model->send_notice_p($insert_id, $course_id);
	        }else{
                $send_mail = 0;
            }
        	...
        	$response['status'] = 'error';
        	$response['send_mail'] = $send_mail;
            $response['message'] = get_phrase('you_do_not_have_permission_to_access_this_programs');
            return json_encode($response);
        }
    }
    public function edit_notice_p($notice_id = ""){
        $notice_details = $this->get_notices($notice_id)->row_array();
        if($this->is_valid_user($notice_details['programs_id'])->num_rows() > 0){
            $data['title'] = htmlspecialchars($this->input->post('notice_title'));
            $data['description'] = htmlspecialchars($this->input->post('notice_description'));
            $data['date_updated'] = time();
            $this->db->where('id', $notice_id);
    ...
    public function get_notice_by_programs_id($course_id = ""){
    	if($course_id > 0){
    		$this->db->where('programs_id', $course_id);
    	}
        $this->db->order_by('id', 'desc');
    	return $this->db->get('noticeboard');
    }
