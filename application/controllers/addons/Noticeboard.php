...
class Noticeboard extends CI_Controller {
    ...
    public function load_noticeboard_form_p($course_id = ""){
        $this->load->view('backend/'.strtolower($this->session->userdata('role')).'/noticeboard_form_p');
    }
    ...
    public function load_notice_list_p($course_id = ""){
        $page_data['notice_list'] = $this->noticeboard_model->get_notice_by_programs_id($course_id)->result_array();
        $this->load->view('backend/'.strtolower($this->session->userdata('role')).'/noticeboard_list_p', $page_data);
    }
    ...
    public function notice_details($notice_id = ""){
     ...
     public function notice_details_p($notice_id = ""){
        $notice_description = $this->noticeboard_model->get_notices($notice_id)->row_array();
        echo '<h5>'.htmlspecialchars_decode($notice_description['title']).'</h5><small>'.htmlspecialchars_decode($notice_description['description']).'</small>';
    }
