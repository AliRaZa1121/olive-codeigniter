...
class User extends CI_Controller
{
    ...
    public function instructor_authorization($method)
    {
        ...
        if ($this->session->userdata('is_instructor') != 1 &&  $this->session->userdata('is_organization') != 1) {
            $unprotected_routes = ['become_an_instructor', 'manage_profile', 'save_course_progress', 'warnings', 'report', 'warning_reply', 'report_submit', 'warning_status', 'get_users_by_course_id'];
    ...
    public function programs()
    {
        if ($this->session->userdata('user_login') == true) {
            ...
            $page_data['selected_instructor_id'] = 'all';
        } else {
            redirect(site_url('login'), 'refresh');
        }
        if (isset($_GET['instructor_id']) && !empty($_GET['instructor_id'])) {
            $page_data['selected_instructor_id'] = $_GET['instructor_id'];
        }

        $page_data['selected_category_id']   = isset($_GET['category_id']) ? $_GET['category_id'] : "all";
        $page_data['selected_price']         = isset($_GET['price']) ? $_GET['price'] : "all";
        $page_data['selected_status']        = isset($_GET['status']) ? $_GET['status'] : "all";
        $page_data['programs']               = $this->crud_model->filter_programs_for_backend($page_data['selected_category_id'], $page_data['selected_instructor_id'], $page_data['selected_price'], $page_data['selected_status']);
        $page_data['page_name']              = 'program-server-side';
        $page_data['categories']             = $this->crud_model->get_categories();
        $page_data['page_title']             = get_phrase('active_consultancy_programs');
        $this->load->view('backend/index', $page_data);
    }

    public function quizes_result()
    {
        exit();
        if ($this->session->userdata('user_login') != true) {
            redirect(site_url('login'), 'refresh');
        ...
        $page_data['courses']                = $this->crud_model->filter_course_for_backend($page_data['selected_category_id'], $page_data['selected_student_id'], $page_data['selected_price'], $page_data['selected_status']);
        $page_data['page_name']              = 'quiz_results';
        $page_data['categories']             = $this->crud_model->get_users();
    ...
    public function course_actions($param1 = "", $param2 = "")
    {
        ...
        if ($param1 == "add") {
            ...
            redirect(site_url('user/courses/' . $course_id), 'refresh');
    ...
    public function programs_actions($param1 = "", $param2 = "")
    {
        if ($this->session->userdata('user_login') == true && $this->session->userdata('is_instructor') == 0) {
            redirect(site_url('login'), 'refresh');
        }

        if ($param1 == "add") {
            $programs_id = $this->crud_model->add_programs();
            redirect(site_url('user/programs_form/edit_programs/' . $programs_id), 'refresh');
        } elseif ($param1 == "edit") {
            ...
            $this->is_the_programs_belongs_to_current_instructor($param2);
            $this->crud_model->change_programs_status('pending', $param2);
            redirect(site_url('user/programs'), 'refresh');
        } elseif ($param1 == 'publish_admin') {
            $this->crud_model->change_programs_status('active', $param2);
            redirect(site_url('user/programs'), 'refresh');
        }
    }
    ...
    public function course_form($param1 = "", $param2 = "")
    {
        ...
        } elseif ($param1 == 'quiz_analytics') {
            $this->is_the_course_belongs_to_current_instructor($param2);

            $page_data['page_name'] = 'quiz_analytics';
            $page_data['course_id'] =  $param2;
            $page_data['page_title'] = get_phrase('quiz_analytics');
            $page_data['languages'] = $this->crud_model->get_all_languages();
            $page_data['categories'] = $this->crud_model->get_categories();
            $this->load->view('backend/index', $page_data);
    ...
    public function programs_form($param1 = "", $param2 = "")
    {
        // if($this->session->userdata('user_login') == true )
        if ($this->session->userdata('user_login') == true && $this->session->userdata('is_instructor') == 0) {
            redirect(site_url('login'), 'refresh');
        ...
        if ($param1 == 'add_programs') {
            $page_data['languages'] = $this->crud_model->get_all_languages();
            $page_data['categories'] = $this->crud_model->get_categories();
            $page_data['page_name'] = 'programs_add';
            $page_data['page_title'] = get_phrase('add_programs');
            $this->load->view('backend/index', $page_data);
        } elseif ($param1 == 'add_course_shortcut') {
            $page_data['languages'] = $this->crud_model->get_all_languages();
            $page_data['categories'] = $this->crud_model->get_categories();
            ...
            $page_data['page_name'] = 'quiz_analytics';
            $page_data['programs_id'] =  $param2;
            $page_data['page_title'] = get_phrase('quiz_analytics');
            $page_data['languages'] = $this->crud_model->get_all_languages();
            $page_data['categories'] = $this->crud_model->get_categories();
            $this->load->view('backend/index', $page_data);
        } elseif ($param1 == 'edit_programs') {

            $page_data['page_name'] = 'programs_edit';
            $page_data['programs_id'] =  $param2;
            $page_data['page_title'] = get_phrase('edit_programs');
            $page_data['languages'] = $this->crud_model->get_all_languages();
            $page_data['categories'] = $this->crud_model->get_categories();
            $this->load->view('backend/index', $page_data);
        }
    ...
    public function quiz_analytics($param1 = "", $param2 = "")
    {
        ...
        $page_data['question']                  = $this->crud_model->filter_students_for_quizes($page_data['selected_student_id']);

        $page_data['page_name']  = 'quiz_results';
        $page_data['question_id']  = $param1;

        $page_data['page_title'] = site_phrase('quiz_results');

        $this->load->view('backend/index', $page_data);
    }
    public function resultsss()
    {
        echo 0;
    }
    ...
    public function programs_sections($param1 = "", $param2 = "", $param3 = "")
    {
        if ($this->session->userdata('user_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
        ...
        if ($param2 == 'add') {
            $this->is_the_programs_belongs_to_current_instructor($param1);
            $this->crud_model->add_section($param1);
            $this->session->set_flashdata('flash_message', get_phrase('section_has_been_added_successfully'));
        } elseif ($param2 == 'edit') {
            $this->is_the_programs_belongs_to_current_instructor($param1, $param3, 'section');
            $this->crud_model->edit_section($param3);
            $this->session->set_flashdata('flash_message', get_phrase('section_has_been_updated_successfully'));
        } elseif ($param2 == 'delete') {
            $this->is_the_programs_belongs_to_current_instructor($param1, $param3, 'section');
        ...
        redirect(site_url('user/programs_form/edit_programs/' . $param1));
    }

    public function lessons_p($course_id = "", $param1 = "", $param2 = "")
    {
        ...
        if ($param1 == 'add') {
            ...
            $this->session->set_flashdata('flash_message', get_phrase('list_has_been_added_successfully'));
            redirect('user/programs_form/edit_programs/' . $course_id);
        } elseif ($param1 == 'edit') {
            $this->crud_model->edit_lesson($param2);
            $this->session->set_flashdata('flash_message', get_phrase('list_has_been_updated_successfully'));
            redirect('user/programs_form/edit_programs/' . $course_id);
        } elseif ($param1 == 'delete') {
            $this->crud_model->delete_lesson($param2);
            $this->session->set_flashdata('flash_message', get_phrase('list_has_been_deleted_successfully'));
            redirect('user/programs_form/edit_programs/' . $course_id);
        } elseif ($param1 == 'filter') {
            redirect('user/lessons/' . $this->input->post('course_id'));
        }
        $page_data['page_name'] = 'programs_lesson_add';
        $page_data['lessons'] = $this->crud_model->get_lessons_p('course', $course_id);
        $page_data['programs_id'] = $course_id;
    ...
    public function warnings()
    {
        ...
        $page_data['page_name'] = 'warnings';
        $page_data['page_title'] = 'Warnings';
        $this->load->view('backend/index.php', $page_data);
    }

    public function warning_reply()
    {
        $this->crud_model->warning_reply();
        redirect(site_url('user/warnings'), 'refresh');
    }

    public function warning_status($id)
    {
        $this->crud_model->warning_status($id);
        $count = $this->crud_model->get_warning_count();
        return $count;
    }

    public function get_users_by_course_id($course_id)
    {
        ...
        //     $users = $this->user_model->get_students_by_course_id($course_id, $user_id);
        // }
        // else{
        //     $users = $this->user_model->get_instructors_by_course_id($course_id, $user_id);
        // }

        if ($this->session->userdata('is_instructor') == 1) {
            $users = $this->user_model->get_users_according_role('instructor', $course_id, $user_id);
        } else if ($this->session->userdata('is_organization') == 1) {
            $users = $this->user_model->get_users_according_role('organization', $course_id, $user_id);
        } else {
            $users = $this->user_model->get_users_according_role('student', $course_id, $user_id);
        }
    ...
    public function quiz_add_marks($id)
    {
        $response = $this->crud_model->add_marks($id);

        echo $response;
    }

    public function join_academy()
    {

        $page_data['organizations'] = $this->user_model->get_organizations()->result_array();

        $page_data['page_name'] = 'join_academy';
        $page_data['page_title'] = 'Join Academy';
        $this->load->view('backend/index.php', $page_data);
    }

    public function join_academy_request()
    {
        $this->user_model->join_academy_request();
        redirect(site_url('user/join_academy'), 'refresh');
    }

    public function requested_academy_list()
    {

        $page_data['requested_list'] = $this->user_model->requested_academy_list();
        $page_data['page_name'] = 'requested_academy_list';
        $page_data['page_title'] = 'Requested Academy List';
        $this->load->view('backend/index.php', $page_data);
    }

    public function joined_academy_list()
    {
        $page_data['joined_list'] = $this->user_model->joined_academy_list();
        $page_data['page_name'] = 'joined_academy_list';
        $page_data['page_title'] = 'Joined Academy List';
        $this->load->view('backend/index.php', $page_data);
    }

    public function quit_academy($id)
    {
        $this->user_model->quit_academy($id);
        echo 'Quit request sent successfully';
    }

    public function joined_instructor()
    {
        $page_data['joined_list'] = $this->user_model->joined_instructor_list();
        $page_data['page_name'] = 'joined_instructor';
        $page_data['page_title'] = 'Joined Instructor';
        $this->load->view('backend/index.php', $page_data);
    }

    public function instructor_list()
    {
        $page_data['instructor_list'] = $this->user_model->instructor_list_for_organization();
        $page_data['page_name'] = 'instructor_list';
        $page_data['page_title'] = 'Instructor Lists';
        $this->load->view('backend/index.php', $page_data);
    }

    public function register_instructor($instructor_id)
    {
        $page_data['instructor_list'] = $this->user_model->register_instructor_in_academy($instructor_id);
        $page_data['page_name'] = 'instructor_list';
        $page_data['page_title'] = 'Instructor Lists';
        return redirect(site_url('user/instructor_list'), 'refresh');
    }

    public function change_academy_status($id, $action)
    {
        $this->user_model->change_academy_status($id, $action);
