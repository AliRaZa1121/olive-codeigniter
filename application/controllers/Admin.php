...
class Admin extends CI_Controller
{
    ...
    public function allcategories($param1 = "", $param2 = "")
    {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
        ...
        if ($param1 == 'add') {
            ...
            redirect(site_url('admin/allcategories'), 'refresh');
        } elseif ($param1 == "edit") {
            ...
            $this->crud_model->delete_category($param2);
            $this->session->set_flashdata('allcategories', get_phrase('data_deleted'));
            redirect(site_url('admin/allcategories'), 'refresh');
        }
        $page_data['page_name'] = 'allcategories';
        $page_data['page_title'] = get_phrase('allcategories');
        $page_data['categories'] = $this->crud_model->all_categories($param2);
        $this->load->view('backend/index', $page_data);
    }
    ...
    public function instructor_form($param1 = "", $param2 = "")
    {
        ...
        if ($param1 == 'add_instructor_form') {
            ...
            $page_data['page_title'] = get_phrase('coach_add');
            $this->load->view('backend/index', $page_data);
        } elseif ($param1 == 'edit_instructor_form') {
            $page_data['page_name'] = 'instructor_edit';
            $page_data['user_id'] = $param2;
            $page_data['page_title'] = get_phrase('coach_edit');
            $this->load->view('backend/index', $page_data);
        }
    }
    ...
    public function instructor_revenue($param1 = "")
    {
        ...
        $page_data['page_title'] = get_phrase('coach_revenue');
        $this->load->view('backend/index', $page_data);
    }
    public function organization_revenue($param1 = "")
    {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
        ...
        $page_data['page_name'] = 'organization_revenue';
        $page_data['payment_history'] = $this->crud_model->get_revenue_by_user_type("", "", 'organization_revenue');
        $page_data['page_title'] = get_phrase('organization_revenue');
        $this->load->view('backend/index', $page_data);
    }
    ...
    public function broad_settings($param1 = "")
    {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
        ...
        $page_data['languages'] = $this->crud_model->get_all_languages();
        $page_data['page_name'] = 'broad_settings';
        $page_data['page_title'] = get_phrase('broad_settings');
        $this->load->view('backend/index', $page_data);
    }
    ...
    public function instructor_settings($param1 = "")
    {
        ...
        if ($param1 == 'update') {
            ...
            $this->session->set_flashdata('flash_message', get_phrase('coach_settings_updated'));
            redirect(site_url('admin/instructor_settings'), 'refresh');
        }

        $page_data['page_name'] = 'instructor_settings';
        $page_data['page_title'] = get_phrase('coach_settings');
        $this->load->view('backend/index', $page_data);
    }
    public function organization_settings($param1 = "")
    {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
        ...
        // CHECK ACCESS PERMISSION
        check_permission('user');
        check_permission('organization');

        if ($param1 == 'update') {
            $this->crud_model->update_organization_settings();
            $this->session->set_flashdata('flash_message', get_phrase('Organization_settings_updated'));
            redirect(site_url('admin/organization_settings'), 'refresh');
        }

        $page_data['page_name'] = 'organization_settings';
        $page_data['page_title'] = get_phrase('organization_settings');
        $this->load->view('backend/index', $page_data);
    }
    ...
    public function paypal_checkout_for_organization_revenue()
    {
        if ($this->session->userdata('admin_login') != 1) {
            redirect(site_url('login'), 'refresh');
        }
        ...
        $page_data['amount_to_pay'] = $this->input->post('amount_to_pay');
        $page_data['payout_id'] = $this->input->post('payout_id');
        $page_data['organization_name'] = $this->input->post('organization_name');
        $page_data['production_client_id'] = $this->input->post('production_client_id');

        // BEFORE, CHECK PAYOUT AMOUNTS ARE VALID
        $payout_details = $this->crud_model->get_payouts($page_data['payout_id'], 'payout')->row_array();
        if ($payout_details['amount'] == $page_data['amount_to_pay'] && $payout_details['status'] == 0) {
            $this->load->view('backend/admin/paypal_checkout_for_organization_revenue', $page_data);
        } else {
            $this->session->set_flashdata('error_message', get_phrase('invalid_payout_data'));
            redirect(site_url('admin/organization_payout'), 'refresh');
        }
    }
    ...
    public function instructor_payout($param1 = "")
    {
        ...
        $page_data['page_title'] = get_phrase('coach_payout');
    ...
    public function organization_application($param1 = "", $param2 = "")
    { // param1 is the status and param2 is the application id
        if ($this->session->userdata('admin_login') != 1) {
            redirect(site_url('login'), 'refresh');
        }
        ...
        $page_data['page_name'] = 'application_list';
        $page_data['page_title'] = get_phrase('organization_application');
        $page_data['approved_applications'] = $this->user_model->get_approved_applications();
        $page_data['pending_applications'] = $this->user_model->get_pending_applications();
        $this->load->view('backend/index', $page_data);
    }
    ...
    // INSTRUCTOR PAYOUT SECTION
    public function organization_payout($param1 = "")
    {
        if ($this->session->userdata('admin_login') != 1) {
            redirect(site_url('login'), 'refresh');
        }
        ...
        $page_data['page_name'] = 'organization_payout';
        $page_data['page_title'] = get_phrase('organization_payout');
        $page_data['completed_payouts'] = $this->crud_model->get_completed_payouts_by_date_range($page_data['timestamp_start'], $page_data['timestamp_end']);
        $page_data['pending_payouts'] = $this->crud_model->get_pending_payouts();
        $this->load->view('backend/index', $page_data);
    }
    ...
    public function sample_video_actions($param1 = "", $param2 = "")
    {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
        ...
        if ($param1 == "add") {
            $sample_video_id = $this->crud_model->add_sample_video();

            redirect(site_url('admin/sample_video_form/edit_sample_video/' . $sample_video_id), 'refresh');
        } elseif ($param1 == "edit") {

            $this->crud_model->update_sample_video($param2);

            redirect(site_url('admin/sample_video_form/edit_sample_video/' . $param2));
        } elseif ($param1 == 'delete') {

            $this->crud_model->delete_sample_video($param2);
            redirect(site_url('admin/sample_videos'), 'refresh');
        }
    }

    public function sample_video_form($param1 = "", $param2 = "")
    {
        ...
        // // CHECK ACCESS PERMISSION
        // check_permission('sample_videos');

        if ($param1 == 'add_sample_video') {
            $page_data['courses'] = $this->crud_model->get_courses();

            $page_data['page_name'] = 'sample_video_add';
            $page_data['page_title'] = 'Add Sample Video';
            $this->load->view('backend/index', $page_data);
        } elseif ($param1 == 'edit_sample_video') {

            $page_data['page_name'] = 'sample_video_edit';
            $page_data['sample_video_id'] = $param2;
            $page_data['page_title'] = 'Edit Sample Video';
    ...
    public function sample_videos()
    {
        ...
        // Courses query is used for deciding if there is any course or not. Check the view you will get it
        $page_data['sample_videos'] = $this->crud_model->get_sample_videos();

        $page_data['page_name'] = 'sample_videos';

        $page_data['page_title'] = 'Sample Videos';
        $this->load->view('backend/index', $page_data);
    }

    public function change_sample_video_status($status, $id)
    {
        $this->crud_model->change_status_sample_video($status, $id);
        redirect(site_url('admin/sample_videos'), 'refresh');
    }
    ...
    public function reports()
    {
        ...
        $page_data['reports'] = $this->crud_model->get_reports();

        $page_data['page_name'] = 'report';

        $page_data['page_title'] = 'User Reports';
        $this->load->view('backend/index', $page_data);
    }

    public function report_actions($action = "", $user_id = "", $id = "")
    {
        ...
        $this->crud_model->update_read_status($report_id);
        if ($action == "suspend") {

            $user = $this->user_model->suspend_user($user_id, "only");

            redirect(site_url('admin/reports'), 'refresh');
        } elseif ($action == "warning") {
            $this->crud_model->add_warning();
            redirect($_SERVER['HTTP_REFERER']);
        } elseif ($action == "terminate") {
            $this->crud_model->terminate_report($id);
            redirect(site_url('admin/reports'), 'refresh');
        } elseif ($action == "ban") {
            $user = $this->user_model->banned_user($user_id, "only");
            redirect($_SERVER['HTTP_REFERER']);
        } elseif ($action == "add_to_featured") {
            $user = $this->user_model->add_to_feature($user_id);
            redirect(site_url('admin/instructors'), 'refresh');
        } elseif ($action == "remove_from_featured") {
            $user = $this->user_model->remove_from_feature($user_id);
            redirect(site_url('admin/instructors'), 'refresh');
        }
    }

    //organizations
    public function organizations($action = "", $id = "")
    {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
        // CHECK ACCESS PERMISSION
        check_permission('organizations');

        if ($action == "add") {

            $this->crud_model->add_organzation(); // PROVIDING TRUE FOR ORGANIZATION
            redirect(site_url('admin/organizations'), 'refresh');
        } elseif ($action == "edit") {
            $this->crud_model->edit_organzation($id);
            redirect(site_url('admin/organizations'), 'refresh');
        } elseif ($action == "delete") {
            $this->crud_model->delete_organzation($id);
            redirect(site_url('admin/organizations'), 'refresh');
        }
    ...
    public function organization_form($action = "", $id = "")
    {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
        ...
        if ($action == 'add_organization_form') {
            $page_data['page_name'] = 'organization_add';
            $page_data['page_title'] = get_phrase('organization_add');
            $this->load->view('backend/index', $page_data);
        } elseif ($action == 'edit_organization_form') {
            $page_data['page_name'] = 'organization_edit';
            $page_data['organization_id'] = $id;
            $page_data['page_title'] = get_phrase('organization_edit');
            $page_data['organization'] = $this->crud_model->get_organization_details_by_id($id);
            $this->load->view('backend/index', $page_data);
        }
    }
    ...
    public function books($action = "", $id = "")
    {
        ...
        // CHECK ACCESS PERMISSION
        check_permission('books');

        if ($action == "add") {

            $this->crud_model->add_book(); // PROVIDING TRUE FOR ORGANIZATION
            redirect(site_url('admin/books'), 'refresh');
        } elseif ($action == "edit") {
            $this->crud_model->edit_book($id);
            redirect(site_url('admin/books'), 'refresh');
        } elseif ($action == "delete") {
        ...
        $page_data['books'] = $this->crud_model->get_books();
        $this->load->view('backend/index', $page_data);
    }

    public function book_form($action = "", $id = "")
    {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
        ...
        if ($action == 'add_book_form') {
            $page_data['page_name'] = 'book_add';
            $page_data['page_title'] = get_phrase('book_add');
            $this->load->view('backend/index', $page_data);
        } elseif ($action == 'edit_book_form') {
            $page_data['page_name'] = 'book_edit';
            $page_data['book_id'] = $id;
            $page_data['page_title'] = get_phrase('book_edit');
            $page_data['book'] = $this->crud_model->get_book_details_by_id($id);
            $this->load->view('backend/index', $page_data);
        }
    }
    ...
    public function contents($type, $action = "", $id = "")
    {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
        ...
        // CHECK ACCESS PERMISSION
        check_permission('contents');

        // check type in enum
        $types_enums = ['articles', 'news', 'events'];
        if (!in_array($type, $types_enums)) {
            $this->session->set_flashdata('error_message', get_phrase('wrong_type_selected'));
            redirect(site_url('admin/dashboard'), 'refresh');
        }

        if ($action == 'add') {

            $response = $this->crud_model->add_content($type);
            if ($response) {
                $this->session->set_flashdata('flash_message', get_phrase('data_added_successfully'));
            } else {
                $this->session->set_flashdata('error_message', get_phrase($type . '_name_already_exists'));
            }
            redirect(site_url('admin/contents/' . $type), 'refresh');
        } elseif ($action == "edit") {
            $response = $this->crud_model->edit_content($type, $id);
            if ($response) {
            ...
            $this->crud_model->delete_content($id);
            $this->session->set_flashdata('flash_message', get_phrase('data_deleted'));
            redirect(site_url('admin/contents/' . $type), 'refresh');
        }
        $page_data['page_name'] = 'contents';
        $page_data['type'] = $type;
        $page_data['page_title'] = get_phrase($type);
        $page_data['contents'] = $this->crud_model->get_contents($type, $id);
        $this->load->view('backend/index', $page_data);
    }

    public function content_form($type, $page = "", $id = "")
    {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
        ...
        if ($page == "add_content") {
            ...
            $page_data['page_name'] = 'add_content';
            $page_data['type'] = $type;
            $page_data['page_title'] = get_phrase('add_' . $type);
        }
        if ($page == "edit_content") {

            $page_data['page_name'] = 'edit_content';
            $page_data['type'] = $type;
            $page_data['page_title'] = get_phrase('edit_' . $type);
            $page_data['content'] = $this->crud_model->get_contents_details_by_id($id);
            $page_data['content_id'] = $id;
        }
    ...
    public function content_settings($page = null, $section = null)
    {
        ...
        if ($page != null) {
            ...
            if ($section != null) {
                ...
                $this->session->set_flashdata('flash_message', get_phrase('content_updated'));
                redirect(site_url('admin/content_settings/' . $page), 'refresh');
            }
        } else {
            $page_data['page_name'] = 'content_all_pages';
            $page_data['page_title'] = get_phrase('all_pages');
            $page_data['all_pages_list'] = $this->crud_model->get_all_pages_list();
        }
        $this->load->view('backend/index', $page_data);
    }
}
